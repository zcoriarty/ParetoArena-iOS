...
struct BuyViewControllerRepresentable: UIViewControllerRepresentable {
    typealias UIViewControllerType = BuyViewController
    var currentStock: CurrentStock
    var actionType: String

    func makeUIViewController(context: Context) -> BuyViewController {
        let storyboard = UIStoryboard(name: "Portfolio", bundle: nil)
        let viewController = storyboard.instantiateViewController(withIdentifier: BuyViewController.identifier) as! BuyViewController
        viewController.object = currentStock
        UserDefaults.standard.set(actionType, forKey: "Buy")
        return viewController
    }

    func updateUIViewController(_ uiViewController: BuyViewController, context: Context) {
    }
}
...
//        self.compimage.setImageForName(self.symbol, backgroundColor: nil, circular: true, textAttributes: nil)
        let price: String = object?.ticker?.dailyBar?.c ?? "0.0"
        ...
        let morePrecisePI = Double(object?.ticker?.dailyBar?.c ?? "100") ?? 100
...
//            self.navigationItem.setTitle("Purchase Stock", subtitle: "")
            ...
            self.overviewView.backgroundColor = UIColor(named: "UpColor")
//            buyline.alpha = 1
//            sellLine.alpha = 0
...
//            self.navigationItem.setTitle("Sell Stock", subtitle: "")
            ...
            self.txtPrice.isUserInteractionEnabled = true
            self.overviewView.backgroundColor = UIColor(.red)
        ...
        if textField == txtPrice {
            ...
            if updatedText != "" || updatedText != "0" {
                if let cost = Double(updatedText) {
                    ...
                    let oneStock = Double(object?.ticker?.dailyBar?.c ?? "1") ?? 1.0
